...
export default function App() {
  ...
  function Next() {
    let totalPages = pokemons.length / perPagePokemon;
  ...
  function handleSelect(e) {
    ...
    let pokemonFilter = [...pokemons].filter(
  ...
  return (
    <div className="App">
      ...
      <div>
        {filteredPokemon.map((filterPoke) => {
          return (
            <div key={filterPoke.id}>
              <img src={filterPoke.image} alt="" />
              <h2>{filterPoke.name}</h2>
              <h5>{filterPoke.description}</h5>
            </div>
          );
        })}
      </div>
